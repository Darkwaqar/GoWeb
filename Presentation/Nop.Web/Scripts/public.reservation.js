﻿var Reservation = {
    ...
    daysToMark: function daysToMark(date) {
        for (i = 0; i < Reservation.availableDates.length; i++) {
            var splitResults = new Date(Reservation.availableDates[i].Date.match(/\d+/)[0] * 1);
            var year = splitResults.getYear();
            var month = splitResults.getMonth();
            var day = splitResults.getDate();
    ...
    daysToMarkTo: function daysToMark(date) {
        for (i = 0; i < Reservation.availableDatesTo.length; i++) {
            var splitResults = new Date(Reservation.availableDatesTo[i].Date.match(/\d+/)[0] * 1);
            var year = splitResults.getYear();
            var month = splitResults.getMonth();
            var day = splitResults.getDate();
    ...
    daysToMarkFrom: function daysToMark(date) {
        for (i = 0; i < Reservation.availableDatesFrom.length; i++) {
            var splitResults = new Date(Reservation.availableDatesFrom[i].Date.match(/\d+/)[0] * 1);
            var year = splitResults.getYear();
            var month = splitResults.getMonth();
            var day = splitResults.getDate();
    ...
    onDatePickerDateChange: function onDatePickerDateChange() {
        ...
        if (selected != null) {
            ...
            for (i = 0; i < Reservation.availableDates.length; i++) {
                ...
                if (selectedYear == year + 1900 && selectedMonth == month + 1 && selectedDay == day) {
                    $("#hoursDiv").append("<li><input type='radio' id='Reservation_" + Reservation.availableDates[i].Id + "' name='Reservation' value='" + Reservation.availableDates[i].Id + "' />" + "<label for='Reservation_" + Reservation.availableDates[i].Id + "'>" + new Date(Reservation.availableDates[i].Date.match(/\d+/)[0] * 1).toLocaleString().substring(11, 16) +"</label></li>");
                }
            }
