...
CREATE PROCEDURE [dbo].[ProductLoadAllPagedAdvance]
(
	...
	@MarkedAsSalesOnly	bit = 0, 	--0 - load all products , 1 - "marked as sales" only
...
BEGIN
	...
	--filter by "marked as sales"
	IF @MarkedAsSalesOnly = 1
	BEGIN
		SET @sql = @sql + '
		AND (p.OldPrice > 0  or p.HasDiscountsApplied = 1)'
	END
	...
	ELSE IF @OrderBy = 20 /* creation date */
		SET @sql_orderby = ' p.[Sold] DESC'
	ELSE IF @OrderBy = 25 /* creation date */
		SET @sql_orderby = ' p.[Viewed] DESC'
	ELSE IF @OrderBy = 30 /* creation date */
		SET @sql_orderby = ' p.[OnSale] DESC'
