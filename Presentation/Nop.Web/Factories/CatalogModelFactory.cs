    ...
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        public virtual void PrepareSortingOptions(CatalogPagingFilteringModel pagingFilteringModel, CatalogPagingFilteringModel command)
        {
            ...
            if (command.OrderBy == null)
                command.OrderBy = allDisabled ? 0 : (activeOptions.Any(x => x.Key == _catalogSettings.DefaultProductSorting) ? _catalogSettings.DefaultProductSorting : activeOptions.First().Key);
        ...
        public virtual CategoryModel PrepareCategoryModel(Category category, CatalogPagingFilteringModel command)
        {
            ...
            //prepare alternative picture model
            var alternativePictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_ALTERNATIVE_PICTURE_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
            model.AlternativePictureModel = _cacheManager.Get(alternativePictureCacheKey, () =>
            {
                var picture = _pictureService.GetPictureById(category.AlternativePictureId);
                var pictureModel = new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                    ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
            ...
            if (_catalogSettings.AllowProductFilter)
            {
            ...
            var products = _productService.SearchProducts(out filterableSpecificationAttributeOptionIds,
                _catalogSettings.AllowProductFilter,
        ...
        public virtual List<CategoryModel> PrepareHomepageCategoryModels()
        {
            ...
            var model = _cacheManager.Get(categoriesCacheKey, () =>
                ...
                .Select(category =>
                {
                    ...
                    //prepare alternative picture model
                    var alternativePictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_ALTERNATIVE_PICTURE_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                    catModel.AlternativePictureModel = _cacheManager.Get(alternativePictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(category.AlternativePictureId);
                        var pictureModel = new PictureModel
                        {
        ...
        /// <summary>
        /// Prepare Featured category models
        /// </summary>
        /// <returns>List of homepage category models</returns>
        public virtual List<CategoryModel> PrepareFeaturedCategoryModels(int vendorId, bool alternativePicture = false)
        {
            var pictureSize = _mediaSettings.CategoryThumbPictureSize;

            string categoriesCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_FEATURED_KEY,
                vendorId,
                alternativePicture,
                string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                pictureSize,
            ...
            var model = _cacheManager.Get(categoriesCacheKey, () =>
                _categoryService.GetAllFeaturedCategoriesByVendorId(vendorId: vendorId)
                .Select(category =>
                {
                    var catModel = new CategoryModel
                    {
                    ...
                    catModel.PictureModel = _cacheManager.Get(categoryPictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(alternativePicture ? category.AlternativePictureId : category.PictureId);
                        var pictureModel = new PictureModel
                        {
                            FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                            ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
        ...
        /// </summary>
        /// <returns>List of homepage category models</returns>
        public virtual List<CategoryModel> PrepareCategoryModels(int vendorId, bool alternativePicture = false)
        {
            var pictureSize = _mediaSettings.CategoryThumbPictureSize;

            string categoriesCacheKey = string.Format(ModelCacheEventConsumer.VENDOR_CATEGORY_KEY,
                vendorId,
                alternativePicture,
                string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                pictureSize,
            ...
            var model = _cacheManager.Get(categoriesCacheKey, () =>
                _categoryService.GetCategoriesByVendorId(vendorId)
                .Select(category =>
                {
                    var catModel = new CategoryModel
                    {
                    ...
                    var subcategories = _categoryService.GetAllCategoriesByParentCategoryId(catModel.Id).Where(x => x.VendorId == vendorId);
                    ...
                    foreach (var subcategory in subcategories)
                    {
                        var subcatModel = new SubCategoryModel
                        {
                            ...
                            Description = subcategory.GetLocalized(x => x.Description),
                            SeName = subcategory.GetSeName(),
                        };

                        //prepare picture model
                        var subCategoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, subcategory.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                        subcatModel.PictureModel = _cacheManager.Get(subCategoryPictureCacheKey, () =>
                        {
                            var picture = _pictureService.GetPictureById(alternativePicture ? subcategory.AlternativePictureId : subcategory.PictureId);
                            var pictureModel = new PictureModel
                            {
                                FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                                ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
        ...
        /// </summary>
        /// <returns>List of homepage category models</returns>
        public virtual List<CollectionModel> PrepareHomepageCollectionModels()
        {

            var pictureSize = _mediaSettings.CategoryThumbPictureSize;

            string categoriesCacheKey = string.Format(ModelCacheEventConsumer.COLLECTION_HOMEPAGE_KEY,
                string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                pictureSize,
                _storeContext.CurrentStore.Id,
                _workContext.WorkingLanguage.Id,
            ...
            var model = _cacheManager.Get(categoriesCacheKey, () =>
                _categoryService.GetAllCollectionDisplayedOnHomePage()
                .Select(category =>
                {
                    var CollectionPicture = _pictureService.GetPictureById(category.CollectionPrictureId);
                    ...
                    var collectionModel = new CollectionModel
                    {
                        ...
                        CollectionName = category.GetLocalized(x => x.CollectionName),
                        CollectionDiscription = category.GetLocalized(x => x.CollectionDiscription),
                    };

                    //prepare picture model
                    var CollectionPictureCacheKey = string.Format(ModelCacheEventConsumer.COLLECTION_PICTURE_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                    collectionModel.CollectionPicture = _cacheManager.Get(CollectionPictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(category.CollectionPrictureId);
                        var pictureModel = new PictureModel
                        {
                            FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                            ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
                            Title = string.Format(_localizationService.GetResource("Media.Category.ImageLinkTitleFormat"), collectionModel.Name),
                            AlternateText = string.Format(_localizationService.GetResource("Media.Category.ImageAlternateTextFormat"), collectionModel.Name)
                        };
                        return pictureModel;
                    });

                    //prepare picture model
                    var CollectionLogoPictureCacheKey = string.Format(ModelCacheEventConsumer.COLLECTION_LOGO_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                    collectionModel.CollectionLogo = _cacheManager.Get(CollectionLogoPictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(category.CollectionLogoId);
                        var pictureModel = new PictureModel
                        {
                            FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                            ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
                    ...
                    //prepare picture model
                    var categoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                    collectionModel.PictureModel = _cacheManager.Get(categoryPictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(category.PictureId);
                        var pictureModel = new PictureModel
                        {
                    ...
                    var CollectionProductCacheKey = string.Format(ModelCacheEventConsumer.COLLECTION_PRODUCT_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                    collectionModel.Products = _cacheManager.Get(CollectionProductCacheKey, () =>
                    {
                        IList<int> categoriesid = new List<int>();
                        categoriesid.Add(category.Id);
                        var products = _productService.SearchProducts(categoryIds: categoriesid).ToList();
                        return _productModelFactory.PrepareProductOverviewModels(products, true, true, pictureSize).ToList();
                    });
        ...
        /// <summary>
        /// Prepare Marketplace category (simple) models
        /// </summary>
        /// <param name="rootCategoryId">Root category identifier</param>
        /// <param name="loadSubCategories">A value indicating whether subcategories should be loaded</param>
        /// <param name="allCategories">All available categories; pass null to load them internally</param>
        /// <returns>List of category (simple) models</returns>
        public virtual List<CategorySimpleModel> PrepareMarketPlaceCategorySimpleModels(int rootCategoryId,
            bool loadSubCategories = true, IList<Category> allCategories = null)
        {
            var result = new List<CategorySimpleModel>();
            ...
            if (allCategories == null)
            {
                ...
                //we implemeneted it this way for performance optimization - recursive iterations (below)
                //this way all categories are loaded only once
                allCategories = _categoryService.GetAllCategories(storeId: _storeContext.CurrentStore.Id).Where(c => c.VendorId == 0).ToList();
            }
            var categories = allCategories.Where(c => c.ParentCategoryId == rootCategoryId);
            foreach (var category in categories)
            {
                var categoryModel = new CategorySimpleModel
                {
                ...
                var pictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, category.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                categoryModel.PictureModel = _cacheManager.Get(pictureCacheKey, () =>
                {
                    var picture = _pictureService.GetPictureById(category.PictureId);
                    var pictureModel = new PictureModel
                    {
                ...
                if (loadSubCategories)
                {
                    var subCategories = PrepareMarketPlaceCategorySimpleModels(category.Id, loadSubCategories, allCategories);
                    categoryModel.SubCategories.AddRange(subCategories);
                }
                if (categoryModel.NumberOfProducts > 0)
                {
                    result.Add(categoryModel);
                }
            }
        ...
        public virtual VendorModel PrepareVendorModel(Vendor vendor, CatalogPagingFilteringModel command)
        {
            ...
            var pictureCacheKey = string.Format(ModelCacheEventConsumer.VENDOR_SHOPBANNER_MODEL_KEY, vendor.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
            model.ShopBanner = _cacheManager.Get(pictureCacheKey, () =>
            {
                var picture = _pictureService.GetPictureById(vendor.ShopBanner);
                var pictureModel = new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                    ImageUrl = _pictureService.GetPictureUrl(picture, pictureSize),
        ...
        /// <param name="vendor">Vendor</param>
        /// <returns>Vendor model</returns>
        public virtual VendorMobileModel PrepareVendorMobileModel(Vendor vendor, bool alternativePicture = false)
        {
            if (vendor == null)
                throw new ArgumentNullException("vendor");
            ...
            var vendorCacheKey = string.Format(ModelCacheEventConsumer.VENDOR_MOBILE_MODEL_KEY, vendor.Id);
            var model = _cacheManager.Get(vendorCacheKey, () =>
            {
                IList<VendorMobileBanner> banners = new List<VendorMobileBanner>();
                foreach (var banner in vendor.VendorBanners.OrderBy(x => x.DisplayOrder))
                {
                    VendorMobileBanner vendorBanner = new VendorMobileBanner
                    {
                        Title = banner.Title,
                        Description = banner.Description,
                        Url = banner.Url,
                        Image = _pictureService.GetPictureUrl(banner.ImageId)
                    };
                    banners.Add(vendorBanner);
                }

                IList<string> VendorNotes = new List<string>();
                foreach (var note in vendor.VendorNotes)
                {
                    VendorNotes.Add(note.Note);
                }

                var appSetting = _vendorService.GetMobileAppSetting(vendor.Id);
                var Social = _vendorService.GetSocialLinks(vendor.Id);

                var allReviews = _vendorService.GetVendorReviews(vendor.Id, null, true);
                var averageRating = allReviews.Any() ? allReviews.Average(x => x.Rating) : 0;
                var vendorRating = new VendorRatingModel()
                {
                ...
                var FeaturedCategories = PrepareFeaturedCategoryModels(vendor.Id, alternativePicture);
                var Categories = PrepareCategoryModels(vendor.Id, alternativePicture);
                var HaveShoptheLook = _productService.SearchProducts(vendorId: vendor.Id, productType: ProductType.GroupedProduct).Count() != 0;
                var vendorMobileModel = new VendorMobileModel
                {
                    Id = vendor.Id,
                    Name = vendor.GetLocalized(x => x.Name),
                    Description = vendor.GetLocalized(x => x.Description),
                    ThemeColor = vendor.GetLocalized(x => x.ThemeColor),
                    ContactDescription = vendor.ContactDescription,
                    AppointmentPictureUrl = _pictureService.GetPictureUrl(appSetting.AppointmentPictureId),
                    CoverPictureURL = _pictureService.GetPictureUrl(vendor.CoverPictureId),
                    MarketPlaceLogoURL = _pictureService.GetPictureUrl(vendor.mpLogo),
                    LogoUrl = _pictureService.GetPictureUrl(vendor.PictureId),
                    ContactNumber = vendor.ContactNumber,
                    AboutUsPageURL = vendor.AboutUsPageUrl,
                    BlogUrl = vendor.BlogUrl,
                    FacebookPageURL = vendor.FacebookPage,
                    LinkedInPageURL = vendor.LinkedInPage,
                    TermAndConditionsUrl = vendor.TermAndConditionsUrl,
                    ReturnAndRefundUrl = vendor.ReturnAndRefundUrl,
                    DeliveryPolicyUrl = vendor.DeliveryPolicyUrl,
                    ShippingInformationUrl = vendor.ShippingInformationUrl,
                    ReturnAndExchangePolicyUrl = vendor.ReturnAndExchangePolicyUrl,
                    PaymentSecurityUrl = vendor.PaymentSecurityUrl,
                    FaqUrl = vendor.FaqUrl,
                    StandAloneApp = vendor.StandAloneAppAndroid,
                    StandAloneAppPackageIdAndroid = vendor.StandAloneAppPackageIdAndroid,
                    StandAloneAppPackageIdIos = vendor.StandAloneAppPackageIdIos,
                    StandAloneAppUrlSchemesIos = vendor.StandAloneAppUrlSchemesIos,
                    FacebookAppId = vendor.FacebookAppId,
                    StandAloneAppIos = vendor.StandAloneAppIos,
                    Banners = banners,
                    VendorNotes = VendorNotes,
                    MobileAppSetting = appSetting.ToModel(),
                    SocialLinks = Social.ToModel(),
                    Rating = vendorRating,
                    Categories = Categories,
                    FeaturedCategories = FeaturedCategories,
                    HaveShoptheLook = HaveShoptheLook,
                    AboutUs = WebUtility.HtmlEncode(vendor.AboutUs),
                };
                return vendorMobileModel;
            });

            model.NumberOfCartItems = (currentCustomer != null ? currentCustomer.ShoppingCartItems.Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart).LimitPerStore(_storeContext.CurrentStore.Id).Count() : 0);
            model.NumberOfWishlistItems = (currentCustomer != null ? currentCustomer.ShoppingCartItems.Where(sci => sci.ShoppingCartType == ShoppingCartType.Wishlist).LimitPerStore(_storeContext.CurrentStore.Id).Count() : 0);
            return model;
        }
        ...
        public virtual List<VendorModel> PrepareVendorAllModels()
        {
            ...
            foreach (var vendor in vendors)
            {
                var vendorModel = new VendorModel
                {
                    ...
                    AllowCustomersToContactVendors = _vendorSettings.AllowCustomersToContactVendors,
                    Brand = vendor.Brand,
                    Designer = vendor.Designer,
                    SheEarns = vendor.SheEarns,
                    TopBrand = vendor.TopBrand,
                    WomenBrand = vendor.Brand,
                    MenBrand = vendor.MenBrand,
                    TopShoesBrand = vendor.TopShoesBrand
        ...
        /// <summary>
        /// Prepare all product tags model
        /// </summary>
        /// <returns>Product tags model</returns>
        public virtual PopularProductTagsModel PrepareAllProductTagsModel()
        {
            //var cacheKey = string.Format(ModelCacheEventConsumer.PRODUCTTAG_ALL_MODEL_KEY, _workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id);
            //var cachedModel = _cacheManager.Get(cacheKey, () =>
            //{
            var model = new PopularProductTagsModel();
            //get all tags
        ...
        /// <param name="command">Catalog paging filtering command</param>
        /// <returns>Search model</returns>
        public virtual SearchModel PrepareMobileSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            if (model == null)
                throw new ArgumentNullException("model");
            ...
            List<SearchModel.CategoryModel> categories = new List<SearchModel.CategoryModel>();
            if (model.vid > 0)
            {
                var categoriesModel = new List<SearchModel.CategoryModel>();
                //all categories

                var allCategories = _categoryService.SearchVendorCategories(vendorId: model.vid);
                foreach (var c in allCategories)
                {
                    categoriesModel.Add(new SearchModel.CategoryModel
                    {
            ...
            {
                string cacheKey = string.Format(ModelCacheEventConsumer.MOBILESEARCH_CATEGORIES_MODEL_KEY);
                categories = _cacheManager.Get(cacheKey, () =>
                {
                    var categoriesModel = new List<SearchModel.CategoryModel>();
                    //all categories
                    var allCategories = _categoryService.GetSearchableCategories();
                    foreach (var c in allCategories)
                    {
                        categoriesModel.Add(new SearchModel.CategoryModel
                        {
            ...
            if (model.asv)
            {
                IPagedList<Vendor> vendors;
                if (model.cid > 0)
                    vendors = _vendorService.GetVendorByCategoryID(model.cid);
                else
            ...
            {
                // only search if query string search keyword is set (used to avoid searching or displaying search term min length error message on /search page load)
                isSearchTermSpecified = _httpContext.Request.Params["SearchTerm"] != null;
            }
            catch
            {
                //the "A potentially dangerous Request.QueryString value was detected from the client" exception could be thrown here when some wrong char is specified (e.g. <)
