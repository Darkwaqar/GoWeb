...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        private VendorSimpleModel PrepareVendorModel(Product product)
        {
            string cacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_VENDOR_MODEL_KEY, product.Id, _storeContext.CurrentStore.Id);
            var model = _cacheManager.Get(cacheKey, () =>
            {
                var vendor = _vendorService.GetVendorById(product.VendorId);
                return new VendorSimpleModel
                {
                    Id = vendor.Id,
        ...
        protected virtual ProductOverviewModel.ProductPriceModel PrepareProductOverviewPriceModel(Product product, bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            {
                case ProductType.MagazineProduct:
                case ProductType.GroupedProduct:
                    ...
                    break;
                case ProductType.ReservationProduct:
                ...
                default:
                    {
                        ...
                        {
                            decimal startPrice = _currencyService.ConvertFromPrimaryStoreCurrency(product.StartPrice, _workContext.WorkingCurrency);
                        ...
                        {
                            decimal highestBid = _currencyService.ConvertFromPrimaryStoreCurrency(product.HighestBid, _workContext.WorkingCurrency);
                            ...
                            if (!product.CustomerEntersPrice)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (finalPriceWithoutDiscount != finalPriceWithDiscount)
                                    {
                                        ...
                                        priceModel.DiscountPercentage = 100 - ((priceModel.PriceValue / finalPriceWithoutDiscount) * 100);
                                        priceModel.PriceWithoutDiscount = _priceFormatter.FormatPrice(finalPriceWithoutDiscount);
                                    }
                                    else if (oldPrice > decimal.Zero)
                                    {
                                        priceModel.PriceWithDiscount = _priceFormatter.FormatPrice(finalPriceWithDiscount);
                                        priceModel.DiscountPercentage = 100 - ((finalPrice / oldPrice) * 100);
                                        priceModel.PriceWithoutDiscount = _priceFormatter.FormatPrice(oldPrice);
                                    }
                                    else
                                    {
                                        priceModel.PriceWithDiscount = _priceFormatter.FormatPrice(finalPriceWithDiscount);
                                        priceModel.PriceWithoutDiscount = _priceFormatter.FormatPrice(finalPrice);
                                    }
        ...
        protected virtual List<PictureModel> PrepareProductOverviewPictureModel(Product product, int? productThumbPictureSize = null, bool SecondPictureOnCatalogPages = false)
        {
            if (product == null)
                throw new ArgumentNullException("product");
            ...
            var cacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_OVERVIEW_PICTURES_MODEL_KEY,
                product.Id, pictureSize, SecondPictureOnCatalogPages, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(),
                _storeContext.CurrentStore.Id);

            List<PictureModel> pictureModelList = _cacheManager.Get(cacheKey, () =>
            {
                var pictures = _pictureService.GetPicturesByProductId(product.Id, SecondPictureOnCatalogPages ? 2 : 1);
                List<PictureModel> pictureModels = new List<PictureModel>();
                foreach (var picture in pictures)
                {
                    var pictureModel = new PictureModel
                    {
                        Id = picture == null ? 0 : picture.Id,
                        ImageUrl = picture == null ? _pictureService.GetDefaultPictureUrl() : _pictureService.GetPictureUrl(picture, pictureSize),
                        FullSizeImageUrl = picture == null ? _pictureService.GetDefaultPictureUrl() : _pictureService.GetPictureUrl(picture)
                    };
                    //"title" attribute
                ...
                if (pictureModels.Count == 0)
                {
                    ...
                    {
                        Id = product.Id,
                        ImageUrl = _pictureService.GetPictureUrl(defaultPicture, pictureSize),
                    ...
                    pictureModels.Add(defaultPictureModel);
                }
        ...
        public virtual void PrepareProductReservation(ProductDetailsModel model, Product product)
        {
            if (product.ProductType == ProductType.ReservationProduct)
            {
                model.AddToCart.IsReservation = true;
                var reservations = EngineContext.Current.Resolve<IProductReservationService>().GetProductReservationsByProductId(product.Id, true, null);
                var inCart = _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.ReservationId > 0).ToList();
        ...
        /// <param name="isAssociatedProduct">Whether the product is associated</param>
        /// <returns>Picture model for the default picture and list of picture models for all product pictures</returns>
        protected virtual List<VideoModel> PrepareProductDetailsVideoModel(Product product)
        {
            if (product == null)
                throw new ArgumentNullException("product");
            ...
            //prepare picture models
            var productVideosCacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_DETAILS_VIDEOS_MODEL_KEY, product.Id, defaultPictureSize, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
            var cachedVideos = _cacheManager.Get(productVideosCacheKey, () =>
            {
                var productName = product.GetLocalized(x => x.Name);

                var videos = _productService.GetProductVideosByProductId(product.Id);
                //all pictures
                var videoModels = new List<VideoModel>();
                foreach (var video in videos)
                {
                    var picture = _pictureService.GetPictureById(video.PictureId);
                    var videoModel = new VideoModel
                    {
                        ...
                        ThumbImageUrl = _pictureService.GetPictureUrl(picture, _mediaSettings.ProductThumbPictureSizeOnProductDetailsPage),
                        FullSizeImageUrl = _pictureService.GetPictureUrl(picture),
                        Title = string.Format(_localizationService.GetResource("Media.Product.VideoLinkTitleFormat.Details"), productName),
                        AlternateText = string.Format(_localizationService.GetResource("Media.Product.VideoAlternateTextFormat.Details"), productName),
                        VideoUrl = video.VideoUrl
                    };
                    //"title" attribute
                    videoModel.Title = !string.IsNullOrEmpty(picture.TitleAttribute) ?
                        picture.TitleAttribute :
                        string.Format(_localizationService.GetResource("Media.Product.VideoLinkTitleFormat.Details"), productName);
                    //"alt" attribute
                    videoModel.AlternateText = !string.IsNullOrEmpty(picture.AltAttribute) ?
                        picture.AltAttribute :
                        string.Format(_localizationService.GetResource("Media.Product.VideoAlternateTextFormat.Details"), productName);
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                var model = new ProductOverviewModel
                {
                    ...
                    OutOfStock = product.GetTotalStockQuantity() <= 0,
                    ...
                    MarkAsSales = product.OldPrice > 0 || product.HasDiscountsApplied,
                ...
                {
                    var pictures = PrepareProductOverviewPictureModel(product, productThumbPictureSize, _catalogSettings.SecondPictureOnCatalogPages);
                    model.DefaultPictureModel = pictures.FirstOrDefault();

                    //prepare second picture model
                    if (_catalogSettings.SecondPictureOnCatalogPages && pictures.Count > 1)
                        model.SecondPictureModel = pictures.Skip(1).Take(1).FirstOrDefault();
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            //videos
            model.VideoModels = PrepareProductDetailsVideoModel(product);
            ...
            if (product.ProductType == ProductType.ReservationProduct)
            {
                PrepareProductReservation(model, product);
            }
            ...
            if (product.ProductType == ProductType.AuctionProduct)
            {
        ...
        /// <summary>
        /// Prepare the customer auction models
        /// </summary>
        /// <param name="customerId">Customer identifier</param>
        /// <param name="productId">Product identifier</param>
        /// <param name="languageId">Language identifier</param>
        /// <returns>Customer Auction Model</returns>
        public virtual CustomerAuctionsModel PrepareCustomerAuctionModel(int customerId, int languageId)
        {
            var model = new CustomerAuctionsModel();

            var customerBids = (_auctionService.GetBidsByCustomerId(customerId)).GroupBy(x => x.ProductId);
            foreach (var item in customerBids)
            {
                var product = _productService.GetProductById(item.Key);
                if (product != null)
                {
                    var amount = _currencyService.ConvertFromPrimaryStoreCurrency(product.HighestBid, _workContext.WorkingCurrency);
                    var maxAmount = _currencyService.ConvertFromPrimaryStoreCurrency(item.Max(x => x.Amount), _workContext.WorkingCurrency);

                    var bid = new CustomerAuctionsModel.CustomerAuctionModel();
                    bid.Ended = product.AuctionEnded;
                    bid.OrderId = item.Where(x => x.Win && x.CustomerId == customerId).FirstOrDefault()?.OrderId;
                    bid.CurrentBidAmount = _priceFormatter.FormatPrice(amount);
                    bid.CurrentBidAmountValue = amount;
                    bid.HighestBidder = product.HighestBidder == customerId;
                    ...
                    bid.ProductName = product.GetLocalized(x => x.Name, languageId);
                    bid.ProductSeName = product.GetSeName(languageId);
                    bid.BidAmountValue = maxAmount;
                    bid.BidAmount = _priceFormatter.FormatPrice(bid.BidAmountValue);
                    model.Items.Add(bid);
                }
            }
