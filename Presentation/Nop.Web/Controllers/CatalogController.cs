...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public virtual ActionResult VendorAll(bool Brand = false, bool Designer = false, bool SheEarns = false)
        {
            ...
            if (Brand == true) return View(_catalogModelFactory.PrepareVendorAllModels().Where(x => x.Brand).ToList());
            if (Designer == true) return View(_catalogModelFactory.PrepareVendorAllModels().Where(x => x.Designer).ToList());
            if (SheEarns == true) return View(_catalogModelFactory.PrepareVendorAllModels().Where(x => x.SheEarns).ToList());
        ...
        #region HomepageCatalog
        [ChildActionOnly]
        public virtual ActionResult HomepageCollection()
        {
            var model = _catalogModelFactory.PrepareHomepageCollectionModels();
            if (!model.Any())
                return Content("");
