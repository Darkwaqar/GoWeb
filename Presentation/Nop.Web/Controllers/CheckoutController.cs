    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual ActionResult Index()
        {
            ...
            if (_orderSettings.OnePageCheckoutEnabled)
                if (_orderSettings.CustomCheckoutEnabled)
                    return RedirectToRoute("CustomCheckout");
                else
        ...
        protected virtual JsonResult OpcLoadStepAfterShippingMethod(List<ShoppingCartItem> cart)
        {
            ...
            if (isPaymentWorkflowRequired)
            {
                ...
                if (_orderSettings.DisableBillingAddressCheckoutStep)
                {
                    if (_addressSettings.CountryEnabled &&
                    _workContext.CurrentCustomer.ShippingAddress != null &&
                    _workContext.CurrentCustomer.ShippingAddress.Country != null)
                    {
                        filterByCountryId = _workContext.CurrentCustomer.ShippingAddress.Country.Id;
                    }
                }
        ...
        public virtual ActionResult OpcSaveShipping(FormCollection form)
        {
            try
            {
                ...
                if (_orderSettings.DisableBillingAddressCheckoutStep && _workContext.CurrentCustomer.ShippingAddress != null)
                {
                    //ship to the same address
        ...
        #region CustomCheckout

        public virtual ActionResult CustomCheckout(FormCollection form)
        {
            //validation
            ...
            var model = new OnePageCustomCheckoutModel();

            Boolean.TryParse(form["JsonRequest"], out bool json);

            if (!cart.Any())
                if (json)
                    return Json(new { error = 1, message = "Your cart is empty" });
                else
                    return RedirectToRoute("HomePage");

            if (!_orderSettings.OnePageCheckoutEnabled)
                if (json)
                    return Json(new { error = 1, message = "One Page Checkout Is Disabled" });
                else
                    return RedirectToRoute("Checkout");
            ...
            model.DisableBillingAddressCheckoutStep = _orderSettings.DisableBillingAddressCheckoutStep;
            ...
            {
                billingAddressModel.SelectedAddressId = _workContext.CurrentCustomer.BillingAddress.Id;
            }
            ...
            if (billingAddressId > 0)
            {
                ...
                if (address != null)
                {
                    _workContext.CurrentCustomer.BillingAddress = address;
                    _customerService.UpdateCustomer(_workContext.CurrentCustomer);
                    billingAddressModel.SelectedAddressId = address.Id;
                }
            }
            else
            {
                ...
                TryUpdateModel(checkoutBillingAddressModel.NewAddress, "BillingNewAddress");

                if (checkoutBillingAddressModel.NewAddress.FirstName != null)
                {
                    //custom address attributes
                    var customAttributes = form.ParseCustomAddressAttributes(_addressAttributeParser, _addressAttributeService);
                    var customAttributeWarnings = _addressAttributeParser.GetAttributeWarnings(customAttributes);
                    ...
                    //validate model
                    TryValidateModel(checkoutBillingAddressModel.NewAddress);
                    if (!ModelState.IsValid)
                    {
                        //model is not valid. redisplay the form with errors
                        billingAddressModel = _checkoutModelFactory.PrepareBillingAddressModel(cart,
                           selectedCountryId: checkoutBillingAddressModel.NewAddress.CountryId,
                           overrideAttributesXml: customAttributes);
                        billingAddressModel.NewAddressPreselected = true;
                        billingAddressModel.SelectedAddressId = 0;
                    }
                    else
                    {
                        //try to find an address with the same values (don't duplicate records)
                        var address = _workContext.CurrentCustomer.Addresses.ToList().FindAddress(
                        checkoutBillingAddressModel.NewAddress.FirstName, checkoutBillingAddressModel.NewAddress.LastName, checkoutBillingAddressModel.NewAddress.PhoneNumber,
                        checkoutBillingAddressModel.NewAddress.Email, checkoutBillingAddressModel.NewAddress.FaxNumber, checkoutBillingAddressModel.NewAddress.Company,
                        checkoutBillingAddressModel.NewAddress.Address1, checkoutBillingAddressModel.NewAddress.Address2, checkoutBillingAddressModel.NewAddress.City,
                        checkoutBillingAddressModel.NewAddress.StateProvinceId, checkoutBillingAddressModel.NewAddress.ZipPostalCode,
                        checkoutBillingAddressModel.NewAddress.CountryId, customAttributes);
                        if (address == null)
                        {
                            //address is not found. let's create a new one
                            address = checkoutBillingAddressModel.NewAddress.ToEntity();
                            address.CustomAttributes = customAttributes;
                            address.CreatedOnUtc = DateTime.UtcNow;
                            //some validation
                            if (address.CountryId == 0)
                        ...
                        _workContext.CurrentCustomer.BillingAddress = address;
                        _customerService.UpdateCustomer(_workContext.CurrentCustomer);
                        billingAddressModel = _checkoutModelFactory.PrepareBillingAddressModel(cart, prePopulateNewAddressWithCustomerFields: true);
                        billingAddressModel.SelectedAddressId = address.Id;
                    }
                }
            }

            model.BillingAddressModel = this.RenderPartialViewToString("OpcBillingAddress", billingAddressModel);
            ...
            if (cart.RequiresShipping())
            {
                #region #region Shipping Address
                //shipping is required
                TryUpdateModel(billingAddressModel);
                if (_shippingSettings.ShipToSameAddress && billingAddressModel.ShipToSameAddress)
                {
                    //ship to the same address
                    _workContext.CurrentCustomer.ShippingAddress = _workContext.CurrentCustomer.BillingAddress;
                    _customerService.UpdateCustomer(_workContext.CurrentCustomer);
                ...
                else
                {
                    ...
                    #region Pickup
                    //pickup point
                    if (_shippingSettings.AllowPickUpInStore && checkoutShippingAddressModel.PickUpInStore)
                    {
                        ...
                        var pickup = form["pickup-points-id"];
                        if (pickup != null)
                        {
                            var pickupPoint = pickup.Split(new[] { "___" }, StringSplitOptions.None);
                            var pickupPoints = _shippingService.GetPickupPoints(_workContext.CurrentCustomer.BillingAddress,
                                _workContext.CurrentCustomer, pickupPoint[1], _storeContext.CurrentStore.Id).PickupPoints.ToList();
                            var selectedPoint = pickupPoints.FirstOrDefault(x => x.Id.Equals(pickupPoint[0]));
                            if (selectedPoint != null)
                            ...
                            shippingAddressModel = _checkoutModelFactory.PrepareShippingAddressModel(prePopulateNewAddressWithCustomerFields: true);
                            shippingAddressModel.PickUpInStore = checkoutShippingAddressModel.PickUpInStore;
                        }
                    }
                    #endregion
                    else
                    {
                        ...
                        {
                            shippingAddressModel.SelectedAddressId = _workContext.CurrentCustomer.ShippingAddress.Id;
                        }
                        int shippingAddressId;
                        int.TryParse(form["shipping_address_id"], out shippingAddressId);
                        ...
                        if (shippingAddressId > 0)
                        {
                            ...
                            if (address != null)
                            {
                                _workContext.CurrentCustomer.ShippingAddress = address;
                                _customerService.UpdateCustomer(_workContext.CurrentCustomer);
                                shippingAddressModel.SelectedAddressId = address.Id;
                            }
                        }
                        else
                        {
                            //new address
                            TryUpdateModel(checkoutShippingAddressModel.NewAddress, "ShippingNewAddress");
                            if (checkoutShippingAddressModel.NewAddress.FirstName != null)
                            {
                                //custom address attributes
                                var customAttributes = form.ParseCustomAddressAttributes(_addressAttributeParser, _addressAttributeService);
                                var customAttributeWarnings = _addressAttributeParser.GetAttributeWarnings(customAttributes);
                                ...
                                //validate model
                                TryValidateModel(checkoutShippingAddressModel.NewAddress);
                                if (!ModelState.IsValid)
                                {
                                    //model is not valid. redisplay the form with errors
                                    shippingAddressModel = _checkoutModelFactory.PrepareShippingAddressModel(
                                        selectedCountryId: checkoutShippingAddressModel.NewAddress.CountryId,
                                        overrideAttributesXml: customAttributes);
                                    shippingAddressModel.SelectedAddressId = 0;
                                    shippingAddressModel.NewAddressPreselected = true;
                                }
                                else
                                {
                                    //try to find an address with the same values (don't duplicate records)
                                    var address = _workContext.CurrentCustomer.Addresses.ToList().FindAddress(
                                        checkoutShippingAddressModel.NewAddress.FirstName, checkoutShippingAddressModel.NewAddress.LastName, checkoutShippingAddressModel.NewAddress.PhoneNumber,
                                        checkoutShippingAddressModel.NewAddress.Email, checkoutShippingAddressModel.NewAddress.FaxNumber, checkoutShippingAddressModel.NewAddress.Company,
                                        checkoutShippingAddressModel.NewAddress.Address1, checkoutShippingAddressModel.NewAddress.Address2, checkoutShippingAddressModel.NewAddress.City,
                                        checkoutShippingAddressModel.NewAddress.StateProvinceId, checkoutShippingAddressModel.NewAddress.ZipPostalCode,
                                        checkoutShippingAddressModel.NewAddress.CountryId, customAttributes);
                                    if (address == null)
                                    {
                                        address = checkoutShippingAddressModel.NewAddress.ToEntity();
                                        address.CustomAttributes = customAttributes;
                                        address.CreatedOnUtc = DateTime.UtcNow;
                                        //little hack here (TODO: find a better solution)
                                        //EF does not load navigation properties for newly created entities (such as this "Address").
                        ...
                        if (model.DisableBillingAddressCheckoutStep && _workContext.CurrentCustomer.ShippingAddress != null)
                        {
                    ...
                    model.ShippingAddressModel = this.RenderPartialViewToString("OpcShippingAddress", shippingAddressModel);
                }
                #endregion
                ...
                model.ShippingMethodModel = this.RenderPartialViewToString("OpcShippingMethods", shippingMethodModel);
            ...
            if (isPaymentWorkflowRequired)
            {
                ...
                if (!String.IsNullOrEmpty(paymentmethod))
                {
                    ...
                    TryUpdateModel(checkoutPaymentMethodModel);

                    //reward points
                    if (_rewardPointsSettings.Enabled)
                    {
                        _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer,
                            SystemCustomerAttributeNames.UseRewardPointsDuringCheckout, checkoutPaymentMethodModel.UseRewardPoints,
                            _storeContext.CurrentStore.Id);
                    }
                ...
                if (!paymentMethodModel.PaymentMethods.Where(x => x.PaymentMethodSystemName == _workContext.CurrentCustomer.GetAttribute<string>(
                    SystemCustomerAttributeNames.SelectedPaymentMethod,
                    _genericAttributeService, _storeContext.CurrentStore.Id)).Any())
                {
                    //select if previous bill method is disable in the country select the first method
                    var selectedPaymentMethodSystemName = paymentMethodModel.PaymentMethods[0].PaymentMethodSystemName;
                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer,
                        SystemCustomerAttributeNames.SelectedPaymentMethod,
                        selectedPaymentMethodSystemName, _storeContext.CurrentStore.Id);
                ...
                //customer have to choose a payment method
                model.PaymentMethodModel = this.RenderPartialViewToString("OpcPaymentMethods", paymentMethodModel);
                ...
                if (paymentMethod != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var paymenInfoModel = _checkoutModelFactory.PreparePaymentInfoModel(paymentMethod);
                            model.PaymenInfoModel = this.RenderPartialViewToString("OpcPaymentInfo", paymenInfoModel);

                            Boolean.TryParse(form["paymentformsubmit"], out bool paymentformsubmit);
                            if (paymentformsubmit)
                            {
                                ...
                                if (ModelState.IsValid)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        var processPaymentRequest = paymentInfo;
                                        if (processPaymentRequest == null)
                                        {
                                            //Check whether payment workflow is required
                                            if (_orderProcessingService.IsPaymentWorkflowRequired(cart))
                                        ...
                                        if (placeOrderResult.Success)
                                        {
                                            ...
                                            var SelectedpaymentMethod = _paymentService.LoadPaymentMethodBySystemName(placeOrderResult.PlacedOrder.PaymentMethodSystemName);
                                            ...
                                            _paymentService.PostProcessPayment(postProcessPaymentRequest);
                                            //success
                                            return Json(new { success = true, successUrl = string.Format("{0}checkout/Completed?orderId={1}", _webHelper.GetStoreLocation(), placeOrderResult.PlacedOrder.Id )});
                                        }
            ...
            var confirmOrderModel = _checkoutModelFactory.PrepareConfirmOrderModel(cart);
            confirmOrderModel.IsEditable = true;
            model.ConfirmOrderModel = this.RenderPartialViewToString("opcConfirmOrder", confirmOrderModel);

            #endregion

            model.ShippingRequired = cart.RequiresShipping();
            model.PaymentRequired = isPaymentWorkflowRequired;
            if (json)
            {
