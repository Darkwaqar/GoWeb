...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        public virtual ActionResult ProductDetails(int productId, int updatecartitemid = 0)
        {
            ...
            EngineContext.Current.Resolve<ICustomerActionEventService>().Viewed(_workContext.CurrentCustomer, this.HttpContext.Request.Path.ToString(), _workContext.CurrentCustomer.GetAttribute<string>(SystemCustomerAttributeNames.LastContinueShoppingPage, _storeContext.CurrentStore.Id));
            _productService.UpdateMostView(productId);
            //model
        ...
        [HttpPost]
        public virtual ActionResult QuickView(int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null || product.Deleted)
                return Json(new
            ...
            //availability dates
            if (!product.IsAvailable() && !(product.ProductType == ProductType.AuctionProduct))
                return Json(new
                {
                    success = false,
                    message = "No product found with the specified ID"
            ...
            if (!product.VisibleIndividually)
            {
                ...
                return Json(new
                {
                    redirect = Url.RouteUrl("Product", new { SeName = parentGroupedProduct.GetSeName() }),
                });
            }
            ...
            _productService.UpdateMostView(productId);
            //model
            var model = _productModelFactory.PrepareProductDetailsModel(product, isAssociatedProduct: false);
        ...
        public virtual ActionResult GetDatesForMonth(int productId, int month, string parameter, int year)
        {
            var allReservations = EngineContext.Current.Resolve<IProductReservationService>().GetProductReservationsByProductId(productId, true, null);
            ...
            var inCart = EngineContext.Current.Resolve<IShoppingCartService>().GetShoppingCart(_storeContext.CurrentStore.Id)
                .Where(x => x.ReservationId > 0).ToList();
        ...
        #region Sales (sales added) products page

        [NopHttpsRequirement(SslRequirement.No)]
        public virtual ActionResult SalesProducts(int CategoryId = 0)
        {
            var products = _productService.SearchProducts(
                storeId: _storeContext.CurrentStore.Id,
                visibleIndividuallyOnly: true,
                categoryIds: new List<int> { CategoryId },
                orderBy: ProductSortingEnum.CreatedOn).Where(product => product.OldPrice > 0 || product.HasDiscountsApplied);
        ...
        {
            //load and cache all product 
            var model = _cacheManager.Get(string.Format(ModelCacheEventConsumer.HOMEPAGE_PRODUCT_IDS_KEY,_workContext.CurrentCustomer.Id, _storeContext.CurrentStore.Id),
                () => {
                    ...
                    //availability dates
                    products = products.Where(p => p.IsAvailable()).ToList();
                    products = products.Where(p => p.VendorId != 0).ToList();
                    //products = products.Where(p => _vendorService.GetVendorById(p.VendorId).IsFeatured).ToList();

                    return _productModelFactory.PrepareProductOverviewModels(products, true, true, productThumbPictureSize);
                });
        ...
        public virtual ActionResult AddProductToCompareList(int productId)
        {
            ...
            //activity log
            _customerActivityService.InsertActivity("PublicStore.AddToCompareList", product.Id, _localizationService.GetResource("ActivityLog.PublicStore.AddToCompareList"), product.Name);
        ...
        #region ProductLine
        [ChildActionOnly]
        public ActionResult ProductLine(int productId, List<int> Categories)
        {
            var products = _productService.SearchProducts(categoryIds: Categories);
            var catids = new List<ProductOverviewModel>();
            var model = _productModelFactory.PrepareProductOverviewModels(products.Where(i => i.Id != productId), true, true).Take(5).ToList();

            if (Request.IsAjaxRequest())
            {
                var obj = from p in model
                          select new
                          {
                              Name = p.Name,
                              SeName = p.SeName,
                              ImageUrl = p.DefaultPictureModel.ImageUrl,
