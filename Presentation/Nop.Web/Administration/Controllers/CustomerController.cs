...
namespace Nop.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual void PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties)
        {
            ...
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
                    ...
                    if (!String.IsNullOrEmpty(customer.Email))
                    {
                        var newsletterSubscriptionVendorIds = new List<int>();
                        foreach (var vendor in allVendors)
                        {
                            var newsletterSubscription = _newsLetterSubscriptionService
                                .GetNewsLetterSubscriptionByEmailAndVendorId(customer.Email, vendor.Id);
                            if (newsletterSubscription != null)
                                newsletterSubscriptionVendorIds.Add(vendor.Id);
                            model.SelectedNewsletterSubscriptionVendorIds = newsletterSubscriptionVendorIds.ToArray();
                        }
                    }
            ...
            //newsletter Vendor subscriptions
            model.AvailableNewsletterSubscriptionVendors = allVendors
                .Select(s => new CustomerModel.VendorModel() { Id = s.Id, Name = s.Name })
                .ToList();
        ...
        [NonAction]
        protected virtual void PrepareDeviceModel(CustomerDeviceModel model, Device device, Customer customer, bool excludeProperties)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
        ...
        [NonAction]
        protected virtual void PrepareNotificationModel(CustomerNotificationModel model, Notification notification, Customer customer, bool excludeProperties)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            if (notification != null)
            {
                if (!excludeProperties)
                {
                    model.Notification = notification.ToModel();
                }
            }
            ...
            model.Notification.AvailableApplications.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Fcm.Application.SelectApplication"), Value = "0" });
            foreach (var c in _fcmApplicationService.GetAllFcmApplication())
                model.Notification.AvailableApplications.Add(new SelectListItem { Text = c.Name, Value = c.Id.ToString(), Selected = (c.Id == model.Notification.ApplicationId) });

            model.Notification.AvailableActions.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Fcm.Application.SelectAction"), Value = "0" });
            foreach (var c in _fcmActionService.SearchFcmActions())
                model.Notification.AvailableActions.Add(new SelectListItem { Text = c.Name, Value = c.Id.ToString(), Selected = (c.Id == model.Notification.ActionId) });


            model.Notification.AvailableVendors.Add(new SelectListItem
            {
            ...
            var vendors = SelectListHelper.GetVendorList(_vendorService, _cacheManager, true);
            foreach (var v in vendors)
                model.Notification.AvailableVendors.Add(v);

            model.Notification.SendImmediately = true;

            if (_workContext.CurrentVendor != null)
            {
                model.Notification.Icon = _workContext.CurrentVendor.mpLogo;
            }


            int prevPictureId = model.Notification.Image;
            //delete an old picture (if deleted or updated)
            if (prevPictureId > 0 && prevPictureId != model.Notification.Image)
            {
            ...
            int prevIconId = model.Notification.Icon;
            //delete an old picture (if deleted or updated)
            if (prevIconId > 0 && prevIconId != model.Notification.Icon)
            {
        ...
        public virtual ActionResult Create(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            {
                var cust2 = _customerService.GetCustomerByEmail(model.Email, _storeContext.CurrentStore.Id);
                if (cust2 != null)
                    ModelState.AddModelError("", "Email is already registered");
            }
            if (!String.IsNullOrWhiteSpace(model.Username) & _customerSettings.UsernamesEnabled)
            {
                var cust2 = _customerService.GetCustomerByUsername(model.Username, _storeContext.CurrentStore.Id);
                if (cust2 != null)
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!String.IsNullOrEmpty(customer.Email))
                {
                    ...
                    foreach (var vendor in allVendors)
                    {
                        var newsletterSubscription = _newsLetterSubscriptionService
                            .GetNewsLetterSubscriptionByEmailAndVendorId(customer.Email, vendor.Id);
                        if (model.SelectedNewsletterSubscriptionVendorIds != null &&
                            model.SelectedNewsletterSubscriptionVendorIds.Contains(vendor.Id))
                        {
                            //subscribed
                            if (newsletterSubscription == null)
                            {
                ...
                {
                    var changePassRequest = new ChangePasswordRequest(model.Email, _storeContext.CurrentStore.Id, false, _customerSettings.DefaultPasswordFormat, model.Password);
                    var changePassResult = _customerRegistrationService.ChangePassword(changePassRequest, null);
        ...
        public virtual ActionResult Edit(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(model.Email))
                    {
                        if (_workContext.CurrentCustomer.Email != customer.Email)
                        {
                            _customerRegistrationService.SetEmail(customer, model.Email, false, _storeContext.CurrentStore.Id);
                        }
                    }
                    else
                    {
                    ...
                    if (_customerSettings.UsernamesEnabled)
                    {
                        ...
                        {
                            _customerRegistrationService.SetUsername(customer, model.Username, _storeContext.CurrentStore.Id);
                        }
                        else
                        {
        ...
        public virtual ActionResult ChangePassword(CustomerModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var changePassRequest = new ChangePasswordRequest(model.Email, _storeContext.CurrentStore.Id,
                    false, _customerSettings.DefaultPasswordFormat, model.Password);
                var changePassResult = _customerRegistrationService.ChangePassword(changePassRequest, customer);
                if (changePassResult.Success)
        ...
        [HttpPost]
        public virtual ActionResult DeviceSelect(int customerId, DataSourceRequest command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedKendoGridJson();
            ...
            var addresses = customer.Devices.OrderByDescending(a => a.CreatedOnUtc).ThenByDescending(a => a.Id).ToList();
            var gridModel = new DataSourceResult
            {
                Data = addresses.Select(x =>
                {
                    var model = x.ToModel();
                    var deviceHtmlSb = new StringBuilder("<div>");
                    if (!String.IsNullOrEmpty(model.DeviceId))
                        deviceHtmlSb.AppendFormat("{0}<br />", Server.HtmlEncode(model.DeviceId));
                    if (!String.IsNullOrEmpty(model.Brand))
                        deviceHtmlSb.AppendFormat("{0}<br />", Server.HtmlEncode(model.Brand));
                    if (!String.IsNullOrEmpty(model.OSVersion))
                        deviceHtmlSb.AppendFormat("{0}<br />", Server.HtmlEncode(model.OSVersion));
                    if (!String.IsNullOrEmpty(model.Carrier))
                        deviceHtmlSb.AppendFormat("{0},", Server.HtmlEncode(model.Carrier));
                    if (!String.IsNullOrEmpty(model.DeviceRam))
                        deviceHtmlSb.AppendFormat("{0}<br />", Server.HtmlEncode(model.DeviceRam));
                    deviceHtmlSb.Append("</div>");
                    model.DeviceHtml = deviceHtmlSb.ToString();
                    return model;
                }),
                Total = addresses.Count
            };
        ...
        [HttpPost]
        public virtual ActionResult DeviceDelete(int id, int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            var device = customer.Devices.FirstOrDefault(a => a.Id == id);
            if (device == null)
                //No customer found with the specified id
                return Content("No customer found with the specified id");
            customer.RemoveDevice(device);
            _customerService.UpdateCustomer(customer);
            //now delete the device record
            _deviceService.DeletesDevice(device);
            return new NullJsonResult();
        }


        public virtual ActionResult DeviceCreate(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            PrepareDeviceModel(model, null, customer, false);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public virtual ActionResult DeviceCreate(CustomerDeviceModel model, FormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            if (ModelState.IsValid)
            {
                var device = model.Device.ToEntity();
                device.UpdatedOnUtc = DateTime.UtcNow;
                customer.Devices.Add(device);
                _customerService.UpdateCustomer(customer);

                SuccessNotification(_localizationService.GetResource("Admin.Customers.Customers.Device.Added"));
                return RedirectToAction("DeviceEdit", new { DeviceId = device.Id, customerId = model.CustomerId });
            }

            //If we got this far, something failed, redisplay form
            PrepareDeviceModel(model, null, customer, true);
            return View(model);
        }


        public virtual ActionResult DeviceEdit(int deviceId, int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            var model = new CustomerDeviceModel();
            PrepareDeviceModel(model, device, customer, false);
            return View(model);
        }

        [HttpPost]
        [ValidateInput(false)]
        public virtual ActionResult DeviceEdit(CustomerDeviceModel model, FormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            var device = _deviceService.GetDeviceById(model.Device.Id);
            if (device == null)
                //No address found with the specified id
                return RedirectToAction("Edit", new { id = customer.Id });
            ...
            if (ModelState.IsValid)
            {
                device = model.Device.ToEntity(device);
                _deviceService.UpdateDevice(device);

                SuccessNotification(_localizationService.GetResource("Admin.Customers.Customers.Devices.Updated"));
                return RedirectToAction("DeviceEdit", new { deviceId = model.Device.Id, customerId = model.CustomerId });
            }

            //If we got this far, something failed, redisplay form
            PrepareDeviceModel(model, device, customer, true);
            return View(model);
        }
        ...
        [HttpPost]
        public virtual ActionResult NotificationSelect(int customerId, DataSourceRequest command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedKendoGridJson();
            ...
            var notificiation = customer.Notifications.OrderByDescending(a => a.CreatedOnUtc).ThenByDescending(a => a.Id).ToList();
            var gridModel = new DataSourceResult
            {
                Data = notificiation.Select(x =>
                {
                    var model = x.ToModel();
                    return model;
                }),
                Total = notificiation.Count
            };
        ...
        [HttpPost]
        public virtual ActionResult NotificationDelete(int id, int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            var notification = customer.Notifications.FirstOrDefault(a => a.Id == id);
            if (notification == null)
                //No customer found with the specified id
                return Content("No notification found with the specified id");
            customer.RemoveNotification(notification);
            _customerService.UpdateCustomer(customer);
            //now delete the device record
            _notificationService.DeletesNotification(notification);
            return new NullJsonResult();
        }


        public virtual ActionResult NotificationCreate(int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            PrepareNotificationModel(model, null, customer, false);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public virtual ActionResult NotificationCreate(CustomerNotificationModel model, FormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            if (ModelState.IsValid)
            {
                var notification = model.Notification.ToEntity();
                notification.UpdatedOnUtc = DateTime.UtcNow;
                customer.Notifications.Add(notification);
                _customerService.UpdateCustomer(customer);

                SuccessNotification(_localizationService.GetResource("Admin.Customers.Customers.Notification.Added"));
                return RedirectToAction("NotificationEdit", new { NotificationId = notification.Id, customerId = model.CustomerId });
            }

            //If we got this far, something failed, redisplay form
            PrepareNotificationModel(model, null, customer, true);
            return View(model);
        }


        public virtual ActionResult NotificationEdit(int notificationId, int customerId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            var model = new CustomerNotificationModel();
            PrepareNotificationModel(model, notification, customer, false);
            return View(model);
        }

        [HttpPost]
        [ValidateInput(false)]
        public virtual ActionResult NotificationEdit(CustomerNotificationModel model, FormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            var notification = _notificationService.GetNotificationById(model.Notification.Id);
            if (notification == null)
                //No address found with the specified id
                return RedirectToAction("Edit", new { id = customer.Id });
            ...
            if (ModelState.IsValid)
            {
                notification = model.Notification.ToEntity(notification);
                _notificationService.UpdateNotification(notification);

                SuccessNotification(_localizationService.GetResource("Admin.Customers.Customers.Notification.Updated"));
                return RedirectToAction("NotificationEdit", new { notificationId = model.Notification.Id, customerId = model.CustomerId });
            }

            //If we got this far, something failed, redisplay form
            PrepareNotificationModel(model, notification, customer, true);
            return View(model);
        }
        ...
        public virtual ActionResult GdprLogList()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            model.AvailableRequestTypes.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
            foreach (var gdprRequestTypeItem in gdprRequestTypeItems)
                model.AvailableRequestTypes.Add(gdprRequestTypeItem);
        ...
        [HttpPost]
        public virtual ActionResult GdprLogList(DataSourceRequest command, GdprLogListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedKendoGridJson();
            ...
            {
                var customer = _customerService.GetCustomerByEmail(model.SearchEmail, model.StoreId);
                if (customer != null)
                    customerId = customer.Id;
                else
                {
                    customerInfo = model.SearchEmail;
                }
            }
            //get requests
            var gdprLog = _gdprService.GetAllLog(
                ...
                requestType: model.SearchRequestTypeId > 0 ? (GdprRequestType?)model.SearchRequestTypeId : null,
                pageIndex: command.Page - 1,
                pageSize: command.PageSize);
        ...
        [HttpPost]
        public virtual ActionResult GdprDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
        ...
        public virtual ActionResult GdprExport(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
