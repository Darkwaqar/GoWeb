    ...
    public partial class ProductController : BaseAdminController
    {
        ...
        protected virtual void PrepareCategoryMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            ...
            foreach (var c in allVendorCategories)
            {
                ...
                model.AvailableVendorCategories.Add(c);
            }

            var allMarketPlaceCategories = SelectListHelper.GetMarketPlaceCategoryList(_categoryService, _cacheManager, true, _workContext.CurrentVendor?.Id ?? 0);
        ...
        protected virtual void SaveCategoryMappings(Product product, ProductModel model)
        {
            ...
            foreach (var vendorCategoryId in model.SelectedVendorCategoryIds)
            {
                model.SelectedCategoryIds.Add(vendorCategoryId);
            }
            ...
            if (_workContext.CurrentVendor != null)
            {
                ...
                {
                    var vendorMappedCategories = _categoryService.GetVendorMappedCategoriesByVendorId(_workContext.CurrentVendor.Id, true).Select(x => x.CategoryId);
                    foreach (var vendorMappedCategory in vendorMappedCategories)
                    {
                        model.SelectedCategoryIds.Add(vendorMappedCategory);
                    }
                }
            }
        ...
        public virtual IEnumerable<ProductModel.BidModel> PrepareBidMode(int productId, int pageIndex, int pageSize)
        {
            var bids = EngineContext.Current.Resolve<IAuctionService>().GetBidsByProductId(productId, pageIndex - 1, pageSize);
        ...
        public virtual ActionResult List()
        {
            ...
            var categories = SelectListHelper.GetVendorCategoriesList(_categoryService, _cacheManager, true, _workContext.CurrentVendor?.Id ?? 0);
            ...
            model.AvailableVendors.Add(new SelectListItem { Text = _localizationService.GetResource("admin.catalog.products.fields.vendor.none"), Value = "-1" });
            ...
            //copy product vendor
            model.CopyProductBulkModel.Published = true;
            model.CopyProductBulkModel.CopyImages = true;
        ...
        public virtual ActionResult ProductList(DataSourceRequest command, ProductListModel model)
        {
            ...
            var products = _productService.SearchProducts(
                ...
                orderBy: (ProductSortingEnum)model.SearchSortOption
            );
        ...
        [ValidateInput(false)]
        public virtual ActionResult ProductVideoAdd(int pictureId, int displayOrder, string videoUrl,
            string overrideAltAttribute, string overrideTitleAttribute,
            int productId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
        ...
        public virtual ActionResult ProductVideoUpdate(ProductModel.ProductVideoModel model)
        {
            ...
            var picture = _pictureService.GetPictureById(productVideo.PictureId);
            if (picture == null)
                throw new ArgumentException("No picture found with the specified id");
        ...
        [HttpPost]
        public virtual ActionResult ImportCsv(ProductListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            {
                ErrorNotification(_localizationService.GetResource("Admin.Catalog.Products.Fields.Vendor.Required"));
                return RedirectToAction("List");
            }
            try
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    _importManager.ImportProductsFromWordPressCsv(file.InputStream, model.SearchVendorId);
                }
                else
                {
                    ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
        ...
        public virtual ActionResult GetCategoriesByVendorId(string vendorId, string productId)
        {
            ...
            var model = new { SelectedCategoryIds = new List<int>(), AvailableVendorCategories = new List<SelectListItem>(), AvailableCategories = new List<SelectListItem>() };
            model.SelectedCategoryIds.AddRange(_categoryService.GetProductCategoriesByProductId(Convert.ToInt32(productId), true).Select(c => c.CategoryId).ToList());
            var allVendorCategories = SelectListHelper.GetVendorCategoriesList(_categoryService, _cacheManager, true, Convert.ToInt32(vendorId));
            foreach (var c in allVendorCategories)
            {
                c.Selected = model.SelectedCategoryIds.Contains(int.Parse(c.Value));
                model.AvailableVendorCategories.Add(c);
            ...
            var allMarketPlaceCategories = SelectListHelper.GetMarketPlaceCategoryList(_categoryService, _cacheManager, true, Convert.ToInt32(vendorId));
            foreach (var c in allMarketPlaceCategories)
            {
                c.Selected = model.SelectedCategoryIds.Contains(int.Parse(c.Value));
                model.AvailableCategories.Add(c);
        ...
        [HttpPost]
        public virtual ActionResult ListReservations(DataSourceRequest command, int productId)
        {
            var product = _productService.GetProductById(productId);

            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedKendoGridJson();
            var _productReservationService = EngineContext.Current.Resolve<IProductReservationService>();
        ...
        [HttpPost]
        public virtual ActionResult GenerateCalendar(int productId, ProductModel.GenerateCalendarModel model)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
            ...
            int _hourFrom = model.StartTime.HasValue ? model.StartTime.Value.Hour : 0;
            int _minutesFrom = model.StartTime.HasValue ? model.StartTime.Value.Minute : 0;
            int _hourTo = model.EndTime.HasValue ? model.EndTime.Value.Hour : 23;
            int _minutesTo = model.EndTime.HasValue ? model.EndTime.Value.Minute : 59;
        ...
        public virtual ActionResult ClearCalendar(int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
        ...
        public virtual ActionResult ClearOld(int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
        ...
        [HttpPost]
        public virtual ActionResult ProductReservationDelete(ProductModel.ReservationModel model)
        {
            var product = _productService.GetProductById(model.ProductId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
        ...
        [HttpPost]
        public virtual ActionResult ListBids(DataSourceRequest command, int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
            ...
            var bidModels = PrepareBidMode(productId, command.Page, command.PageSize);
            var gridModel = new DataSourceResult
            {
                Data = bidModels.ToList(),
                Total = bidModels.Count()
            };
            return Json(gridModel);
        }

        [HttpPost]
        public virtual ActionResult BidDelete(ProductModel.BidModel model)
        {
            var product = _productService.GetProductById(model.ProductId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id");
            ...
            var _AuctionService = EngineContext.Current.Resolve<IAuctionService>();
            ...
            if (toDelete != null)
            {
                ...
                {
                    //activity log
                    _customerActivityService.InsertActivity("DeleteBid", _localizationService.GetResource("ActivityLog.DeleteBid"), product.Name);
                    //delete bid
        ...
        #region Copy Product Bulk

        [HttpPost]
        public virtual ActionResult CopyBulkProduct(ProductListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageVendors))
                return AccessDeniedView();

            if (model.CopyProductBulkModel.FromVendor == 0 || model.CopyProductBulkModel.ToVendor == 0)
            {
                ErrorNotification("Please Select Vendor Id");
                return RedirectToAction("List");
            }
            var copyModel = model.CopyProductBulkModel;
            try
            {
                var copyProducts = _productService.SearchProducts(vendorId: model.CopyProductBulkModel.FromVendor, showHidden: true);

                foreach (var product in copyProducts)
                {
                    var newProduct = _copyProductService.CopyProduct(product,
                    copyModel.ToVendor, copyModel.Published, copyModel.CopyImages);

                    SuccessNotification(newProduct.Name + _localizationService.GetResource("Admin.Catalog.Products.Copied"));
                }
                return RedirectToAction("List");
            }
            catch (Exception exc)
