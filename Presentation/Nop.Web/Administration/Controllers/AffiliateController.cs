...
namespace Nop.Admin.Controllers
{
    public partial class AffiliateController : BaseAdminController
    {
        ...
        [HttpPost]
        public virtual ActionResult AffiliatedCouponList(int affiliateId, DataSourceRequest command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageAffiliates))
                return AccessDeniedKendoGridJson();
            ...
            var gridModel = new DataSourceResult
            {
                Data = Coupons.Select(coupon =>
                {
                    var couponModel = new AffiliateModel.AffiliatedCouponModel();
                    couponModel.Id = coupon.Id;
                    couponModel.Code = coupon.CouponCouponCode;
                    return couponModel;
                }),
