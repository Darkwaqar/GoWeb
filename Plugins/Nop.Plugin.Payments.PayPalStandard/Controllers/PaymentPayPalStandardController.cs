...
namespace Nop.Plugin.Payments.PayPalStandard.Controllers
{
    public class PaymentPayPalStandardController : BasePaymentController
    {
        ...
        public ActionResult PDTHandler(FormCollection form)
        {
            ...
            if (processor.GetPdtDetails(tx, out values, out response))
            {
                ...
                if (order != null)
                {
                    ...
                    if (payPalStandardPaymentSettings.PdtValidateOrderTotal && orderTotalSentToPayPal.HasValue && mc_gross != orderTotalSentToPayPal.Value && order.CustomerCurrencyCode != "PKR")
                    {
                            ...
                            if (order != null)
                            {
                                ...
                                switch (newPaymentStatus)
                                {
                                    ...
                                    case PaymentStatus.Authorized:
                                        {
                                            //validate order total
                                            if (Math.Round(mc_gross, 2).Equals(Math.Round(order.OrderTotal, 2)) || order.CustomerCurrencyCode == "PKR")
                                            {
                                                //valid
                                    ...
                                    case PaymentStatus.Refunded:
                                        {
                                            ...
                                            if (totalToRefund > 0 && Math.Round(totalToRefund, 2).Equals(Math.Round(order.OrderTotal, 2)) || order.CustomerCurrencyCode == "PKR")
                                            {
                                                //refund
        ...
        public ActionResult CancelOrder(FormCollection form)
        {
            ...
            return RedirectToAction("Index", "Checkout", new { area = "" });
        }
    }
}