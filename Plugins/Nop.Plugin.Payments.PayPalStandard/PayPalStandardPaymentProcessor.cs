    ...
    public class PayPalStandardPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        private string GenerationRedirectionUrl(PostProcessPaymentRequest postProcessPaymentRequest, bool passProductNamesAndTotals)
        {
            ...
            else
            {
                ...
                if (postProcessPaymentRequest.Order.CustomerCurrencyCode == "PKR")
                {
                    var USDCurrency = _currencyService.GetCurrencyByCode("USD");
                    orderTotal = Math.Round(_currencyService.ConvertCurrency(postProcessPaymentRequest.Order.OrderTotal, Convert.ToDecimal(USDCurrency.Rate)), 2);
                }
                else {
                    //var orderTotal = Math.Round(postProcessPaymentRequest.Order.OrderTotal, 2);
                    var currency = _currencyService.GetCurrencyByCode(postProcessPaymentRequest.Order.CustomerCurrencyCode);
                    orderTotal = Math.Round(_currencyService.ConvertCurrency(postProcessPaymentRequest.Order.OrderTotal, Convert.ToDecimal(currency.Rate)), 2);
                }
            ...
            builder.Append(string.Format("&no_note=1&currency_code={0}", HttpUtility.UrlEncode(postProcessPaymentRequest.Order.CustomerCurrencyCode == "PKR" ? "USD" : postProcessPaymentRequest.Order.CustomerCurrencyCode)));
            ...
            builder.AppendFormat("&zip={0}", HttpUtility.UrlEncode(postProcessPaymentRequest.Order.BillingAddress.ZipPostalCode == null ? "94533" : postProcessPaymentRequest.Order.BillingAddress.ZipPostalCode));
