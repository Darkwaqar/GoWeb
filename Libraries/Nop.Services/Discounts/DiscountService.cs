...
namespace Nop.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        public virtual string GenerateDiscountCardCode()
        {
            int length = 13;
        ...
        public DiscountValidationResult ValidateDiscount(Discount discount, Customer customer, string couponCodeToValidate)
        {
                ...
                discount.DiscountType == DiscountType.AssignedToOrderTotal)
            {
                ...
                if (hasGiftCards)
                {
                    result.Errors.Add(_localizationService.GetResource("ShoppingCart.Discount.CannotBeUsedWithGiftCards"));
                    return result;
                }
            }
            ...
            if (discount.StartDateUtc.HasValue)
            {
                ...
                if (startDate.CompareTo(now) > 0)
                {
                    result.Errors.Add(_localizationService.GetResource("ShoppingCart.Discount.NotStartedYet"));
                    return result;
                }
            }
            if (discount.EndDateUtc.HasValue)
            {
                ...
                if (endDate.CompareTo(now) < 0)
                {
                    result.Errors.Add(_localizationService.GetResource("ShoppingCart.Discount.Expired"));
                    return result;
                }
            }
                ...
                case DiscountLimitationType.NTimesPerCustomer:
                    {
                        if (customer.IsRegistered())
                        {
                            ...
                            if (usedTimes >= discount.LimitationTimes)
                            {
                                result.Errors.Add(_localizationService.GetResource("ShoppingCart.Discount.CannotBeUsedAnymore"));
                                return result;
                            }
                        }
                    }
            ...
            foreach (var req in requirements)
            {
                ...
                if (!ruleResult.IsValid)
                {
                    result.Errors.Add(ruleResult.UserError);
                    return result;
                }
            }
