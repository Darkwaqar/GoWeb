...
namespace Nop.Services.Forums
{
    ...
    public partial class ForumService : IForumService
    {
        ...
        /// {2} : pageSize
        /// </remarks>
        private const string FORUM_INBOX_KEY = "Nop.forum.inbox-{0}-{1}-{2}";
        ...
        /// </remarks>
        /// Converstation
        private const string FORUM_CONVERSTATION_KEY = "Nop.forum.conversation-{0}-{1}-{2}-{3}";

        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string INBOX_PATTERN_KEY = "Nop.forum.inbox-{0}-";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string CONVERSTATION_PATTERN_KEY = "Nop.forum.conversation-{0}-{1}-";
        ...
        public virtual void DeletePrivateMessage(PrivateMessage privateMessage)
        {
            ...
            //cache
            _cacheManager.RemoveByPattern(string.Format(INBOX_PATTERN_KEY, privateMessage.FromCustomerId));
            _cacheManager.RemoveByPattern(string.Format(CONVERSTATION_PATTERN_KEY, privateMessage.FromCustomerId, privateMessage.ToCustomerId));
        ...
        /// <param name="pageSize">Page size</param>
        /// <returns>Private messages</returns>
        public virtual IPagedList<PrivateMessage> GetInbox(int storeId, int CustomerId,
            bool? isRead, bool? isDeletedByAuthor, bool? isDeletedByRecipient,
            string keywords, int pageIndex = 0, int pageSize = int.MaxValue)
        {

            string key = string.Format(FORUM_INBOX_KEY, CustomerId, pageIndex, pageSize);
            return _cacheManager.Get(key, () =>
            {
                var query = _forumPrivateMessageRepository.Table;
                if (storeId > 0)
                    query = query.Where(pm => storeId == pm.StoreId);
                if (CustomerId > 0)
                    query = query.Where(pm => CustomerId == pm.ToCustomerId || CustomerId == pm.FromCustomerId);
                if (isRead.HasValue)
                    query = query.Where(pm => isRead.Value == pm.IsRead);
                if (isDeletedByAuthor.HasValue)
                    query = query.Where(pm => isDeletedByAuthor.Value == pm.IsDeletedByAuthor);
        ...
        /// <param name="pageSize">Page size</param>
        /// <returns>Private messages</returns>
        public virtual IPagedList<PrivateMessage> GetConverstation(int storeId, int fromCustomerId,
            int toCustomerId, bool? isRead, bool? isDeletedByAuthor, bool? isDeletedByRecipient,
            string keywords, int pageIndex = 0, int pageSize = int.MaxValue)
        {

            string key = string.Format(FORUM_CONVERSTATION_KEY, fromCustomerId, toCustomerId, pageIndex, pageSize);
            return _cacheManager.Get(key, () =>
            {
                var query = _forumPrivateMessageRepository.Table;
                if (storeId > 0)
                    query = query.Where(pm => storeId == pm.StoreId);
                if (toCustomerId > 0)
                    query = query.Where(pm => (toCustomerId == pm.ToCustomerId && fromCustomerId == pm.FromCustomerId) || (toCustomerId == pm.FromCustomerId && fromCustomerId == pm.ToCustomerId));
                if (isRead.HasValue)
                    query = query.Where(pm => isRead.Value == pm.IsRead);
                if (isDeletedByAuthor.HasValue)
                    query = query.Where(pm => isDeletedByAuthor.Value == pm.IsDeletedByAuthor);
