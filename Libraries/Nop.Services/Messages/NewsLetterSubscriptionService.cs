...
namespace Nop.Services.Messages
{
    ...
    public class NewsLetterSubscriptionService : INewsLetterSubscriptionService
    {
        ...
        /// <param name="vendorId">Store identifier. 0 to load all records.</param>
        ...
        public virtual IPagedList<NewsLetterSubscription> GetAllNewsLetterSubscriptions(string email = null,
            ...
            int storeId = 0, int vendorId = 0, bool? isActive = null, int customerRoleId = 0,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            if (customerRoleId == 0)
            {
                ...
                if (vendorId > 0)
                    query = query.Where(nls => nls.VendorId == vendorId);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (vendorId > 0)
                        query = query.Where(x => x.NewsletterSubscribers.VendorId == vendorId);
                    if (isActive.HasValue)
        ...
        public NewsLetterSubscription GetNewsLetterSubscriptionByEmailAndVendorId(string email, int vendorId)
        {
            if (!CommonHelper.IsValidEmail(email))
                return null;
            ...
            var newsLetterSubscriptions = from nls in _subscriptionRepository.Table
                                          where nls.Email == email && nls.VendorId == vendorId
                                          orderby nls.Id
                                          select nls;
