    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        protected Dictionary<string, IEnumerable<string>> AllowedTokens
        {
            get
            {
                ...
                _allowedTokens.Add(TokenGroupNames.ShoppingCart, new[]
                {
                    "%ShoppingCart.Product(s)%",
                    "%ShoppingCart.Product(s).WithPicture%",
                    "%Wishlist.Product(s)%",
                    "%Wishlist.Product(s).WithPicture%" ,
                    "%ShoppingCart.PersonalMessage%",
                    "%ShoppingCart.CustimerEmail%"
                });

                _allowedTokens.Add(TokenGroupNames.Auction, new[]
                {
                    "%Auction.Price%",
                    "%Auction.EndTime%",
                });
        ...
        /// <param name="withPicture">With or without Picture</param>
        /// <param name="customer">customer</param>
        /// <param name="languageId">Language identifier</param>
        /// <returns>HTML table of products</returns>
        protected virtual string ProductListToHtmlTable(bool cart, bool withPicture, Customer customer, int languageId)
        {
            string result;
                ...
                customer.ShoppingCartItems.Where(x => x.ShoppingCartType == ShoppingCartType.Wishlist))
            {
                ...
                if (withPicture)
                {
                    ...
                    {
                        pictureUrl = pictureService.GetPictureUrl(product.ProductPictures.OrderBy(x => x.DisplayOrder).FirstOrDefault().PictureId, _templatesSettings.PictureSize);
                    }
                ...
                {
                    sb.AppendLine("<br />");
                    string attributeDescription = productAttributeFormatter.FormatAttributes(product, item.AttributesXml, customer);
        ...
        public virtual void AddStoreTokens(IList<Token> tokens, Store store, EmailAccount emailAccount)
        {
            ...
            tokens.Add(new Token("InstagramLink.URL", _storeInformationSettings.InstagramLink));
            tokens.Add(new Token("LinkedInLink.URL", _storeInformationSettings.LinkedInLink));
            tokens.Add(new Token("PinterestLink.URL", _storeInformationSettings.PinterestLink));
            tokens.Add(new Token("PlayStoreLink.URL", _storeInformationSettings.PlayStoreLink));
            tokens.Add(new Token("AppStoreLink.URL", _storeInformationSettings.AppStoreLink));
        ...
        public virtual void AddCouponTokens(IList<Token> tokens, Coupon coupon)
        {
            tokens.Add(new Token("Coupon.SenderName", coupon.SenderName));
            tokens.Add(new Token("Coupon.SenderEmail", coupon.SenderEmail));
            tokens.Add(new Token("Coupon.RecipientName", coupon.RecipientName));
            tokens.Add(new Token("Coupon.RecipientEmail", coupon.RecipientEmail));
            tokens.Add(new Token("Coupon.Amount", _priceFormatter.FormatPrice(coupon.Amount, true, false)));
            tokens.Add(new Token("Coupon.CouponCode", coupon.CouponCouponCode));

            var giftCardMesage = !String.IsNullOrWhiteSpace(coupon.Message) ?
                HtmlHelper.FormatText(coupon.Message, false, true, false, false, false, false) : string.Empty;

            tokens.Add(new Token("Coupon.Message", giftCardMesage, true));

            //event notification
            _eventPublisher.EntityTokensAdded(coupon, tokens);
        }
        ...
        public virtual void AddShoppingCartTokens(IList<Token> tokens, Customer customer, Store store, int languageId,
           string personalMessage = "", string customerEmail = "")
        {

            tokens.Add(new Token("ShoppingCart.Product(s)", ProductListToHtmlTable(true, false, customer, languageId)));
            tokens.Add(new Token("ShoppingCart.Product(s).WithPicture", ProductListToHtmlTable(true, true, customer, languageId)));
            tokens.Add(new Token("Wishlist.Product(s)", ProductListToHtmlTable(false, false, customer, languageId)));
            tokens.Add(new Token("Wishlist.Product(s).WithPicture", ProductListToHtmlTable(false, true, customer, languageId)));
            tokens.Add(new Token("ShoppingCart.PersonalMessage", personalMessage));
            tokens.Add(new Token("ShoppingCart.CustimerEmail", customerEmail));
        ...
        /// <param name="attrInfo">attrInfo</param>
        public virtual void AddContactTokens(IList<Token> tokens, string senderEmail, string senderName, string body, string attrInfo)
        {
            ...
            tokens.Add(new Token("ContactUs.AttrInfo", attrInfo, true));
        ...
        public virtual void AddVendorTokens(IList<Token> tokens, Vendor vendor)
        {
            ...
            var vendorAttributesXml = vendor.GetAttribute<string>(SystemVendorAttributeNames.VendorAttributes); ;
        ...
        /// <summary>
        /// Add vendor review tokens
        /// </summary>
        /// <param name="tokens">List of already added tokens</param>
        /// <param name="vendor">Vendor</param>
        /// <param name="product">Product</param>
        /// <param name="vendorReview">Product review</param>
        public virtual void AddVendorReviewTokens(IList<Token> tokens, Vendor vendor, Product product, VendorReview vendorReview)
        {
            tokens.Add(new Token("ProductReview.VendorName", vendor.Name));
            tokens.Add(new Token("ProductReview.ProductName", product.Name));
            tokens.Add(new Token("ProductReview.ReviewText", vendorReview.ReviewText));
        ...
        /// <summary>
        /// Add product appointment tokens
        /// </summary>
        /// <param name="tokens">List of already added tokens</param>
        /// <param name="productAppointment">Product appointment</param>
        public virtual void AddProductAppointmentTokens(IList<Token> tokens, ProductAppointment productAppointment)
        {
            tokens.Add(new Token("ProductAppointment.ProductName", productAppointment.Product.Name));

            //event notification
            _eventPublisher.EntityTokensAdded(productAppointment, tokens);
        }
        ...
        {
            var primaryStoreCurrencyCode = _currencyService.GetCurrencyById(_currencySettings.PrimaryStoreCurrencyId).CurrencyCode;
            tokens.Add(new Token("Auction.Price", _priceFormatter.FormatPrice(bid.Amount, true, primaryStoreCurrencyCode, false, _workContext.WorkingLanguage)));
            tokens.Add(new Token("Auction.EndTime", _dateTimeHelper.ConvertToUserTime(product.AvailableEndDateTimeUtc.Value, DateTimeKind.Utc).ToString()));
        ...
        /// <summary>
        /// Get collection of allowed (supported) message tokens for customer reminder
        /// </summary>
        /// <returns>Collection of allowed (supported) message tokens for customer reminder</returns>
        public virtual IEnumerable<string> GetListOfCustomerReminderAllowedTokens(CustomerReminderRuleEnum rule)
        {
            var additionTokens = new CustomerReminderAdditionTokensAddedEvent();
            _eventPublisher.Publish(additionTokens);

            var allowedTokens = GetListOfAllowedTokens(new[] { TokenGroupNames.CustomerTokens }).ToList();

            if (rule == CustomerReminderRuleEnum.AbandonedCart)
            {
                allowedTokens.AddRange(new[] { TokenGroupNames.ShoppingCart });
            }

            if (rule == CustomerReminderRuleEnum.CompletedOrder || rule == CustomerReminderRuleEnum.UnpaidOrder)
            {
                allowedTokens.AddRange(new[] { TokenGroupNames.OrderTokens });
            }
            allowedTokens.AddRange(additionTokens.AdditionTokens);
