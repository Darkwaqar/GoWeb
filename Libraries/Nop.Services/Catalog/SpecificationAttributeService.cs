...
namespace Nop.Services.Catalog
{
    ...
    public partial class SpecificationAttributeService : ISpecificationAttributeService
    {
        ...
        public SpecificationAttribute GetSpecificationAttributeByOptionId(int specificationAttributeOptionId)
        {
            if (specificationAttributeOptionId == 0)
                return null;

            var query = from sa in _specificationAttributeRepository.Table
                        join so in _specificationAttributeOptionRepository.Table on sa.Id equals so.SpecificationAttributeId
                        where so.Id == specificationAttributeOptionId
                        select sa;
