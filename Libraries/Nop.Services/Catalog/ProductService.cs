...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <param name="productVideoRepository">Product video repository</param>
        ...
        /// <summary>
        /// Gets all vendor products displayed on the home page
        /// </summary>
        /// <returns>Products</returns>
        public virtual IList<Product> GetAllVendorHomePageProduct(int vendorId)
        {
            var query = _productRepository.Table;
            query = query.Where(p => !p.Deleted && p.Published && p.VisibleIndividually && p.VendorId == vendorId && p.ShowOnHomePage);
            var products = query.ToList();
            return products;
        }
        ...
        public virtual void UpdateMostView(int productId)
        {
        ...
        public virtual void UpdateSold(int productId, int qty)
        {
            var product = GetProductById(productId);
            ...
            bool? overridePublished = null)
        {
            ...
            else
            {
                ...
                {
                    //on sale
                    query = query.OrderByDescending(x => x.OnSale);
                }
                else
                {
        ...
        /// <param name="visibleIndividuallyOnly">A values indicating whether to load only products marked as "visible individually"; "false" to load all records; "true" to load "visible individually" only</param>
        /// <param name="markedAsNewOnly">A values indicating whether to load only products marked as "new"; "false" to load all records; "true" to load "marked as new" only</param>
        /// <param name="markedAsSalesOnly">A values indicating whether to load only products marked as "sales"; "false" to load all records; "true" to load "marked as new" only</param>
        /// <param name="featuredProducts">A value indicating whether loaded products are marked as featured (relates only to categories and manufacturers). 0 to load featured products only, 1 to load not featured products only, null to load all products</param>
        /// <param name="priceMin">Minimum price; null to load all records</param>
        /// <param name="priceMax">Maximum price; null to load all records</param>
        /// <param name="productTagId">Product tag identifier; 0 to load all records</param>
        ...
        /// </param>
        /// <returns>Products</returns>
        public virtual IPagedList<Product> SearchProductsAdvance(
            out IList<int> filterableSpecificationAttributeOptionIds,
            bool loadFilterableSpecificationAttributeOptionIds = false,
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            ...
            bool? overridePublished = null)
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                var pMarkedAsSalesOnly = _dataProvider.GetParameter();
                pMarkedAsSalesOnly.ParameterName = "MarkedAsSalesOnly";
                pMarkedAsSalesOnly.Value = markedAsSalesOnly;
                pMarkedAsSalesOnly.DbType = DbType.Int32;
            ...
            else
            {
                ...
                if (markedAsSalesOnly)
                {
                    query = query.Where(p =>
                        (p.OldPrice > 0 || p.HasDiscountsApplied));
                }
                if (productType.HasValue)
                {
                    var productTypeId = (int)productType.Value;
                ...
                if (vendorId < 0)
                {
                    query = query.Where(p => p.VendorId == 0);
                }
        ...
        /// <summary>
        /// Gets products by product specification attribute
        /// </summary>
        /// <param name="specificationAttributeOptionId">Product attribute identifier</param>
        /// <param name="pageIndex">Page index</param>
        /// <param name="pageSize">Page size</param>
        /// <returns>Products</returns>
        public virtual IPagedList<Product> GetProductsByProductSpecificationId(int specificationAttributeOptionId,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _productRepository.Table;
            query = query.Where(x => x.ProductSpecificationAttributes.Any(y => y.SpecificationAttributeOptionId == specificationAttributeOptionId));
            query = query.Where(x => !x.Deleted);
            query = query.OrderBy(x => x.Name);
        ...
        public virtual void UpdateIntervalProperties(int productId, int interval, IntervalUnit intervalUnit, bool includeBothDates)
        {
            var product = GetProductById(productId);
            if (product == null)
                throw new ArgumentNullException("product");
            ...
            product.IncBothDate = includeBothDates;
