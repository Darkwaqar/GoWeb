...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        public virtual decimal GetShoppingCartAdditionalShippingCharge(IList<ShoppingCartItem> cart)
        {
            ...
            Dictionary<int, decimal> vendors = new Dictionary<int, decimal>();
            foreach (var sci in cart)
            {
                ...
                if (!vendors.ContainsKey(sci.Product.VendorId))
                {
                    vendors.Add(sci.Product.VendorId, sciSubTotal);
                }
                else
                {
                    vendors[sci.Product.VendorId] = vendors[sci.Product.VendorId] + sciSubTotal;
                }
            }
            foreach (var v in vendors)
            {
                if (v.Key != 0)
                {
                    var vendor = _vendorService.GetVendorById(v.Key);
                    if (vendor.FreeShippingOverXEnabled)
                    {
                        if (vendor.FreeShippingOverXValue > v.Value)
                        {
                            additionalShippingCharge += vendor.ShippingCharge;
                        }
                    }
                    else
                    {
        ...
        public virtual bool IsFreeShipping(IList<ShoppingCartItem> cart, decimal? subTotal = null)
        {
            ...
            if (_shippingSettings.StoreFreeShippingOverXEnabled)
            {
