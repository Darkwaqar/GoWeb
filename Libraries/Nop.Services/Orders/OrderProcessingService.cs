...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// <param name="productReservationService">ProductReservation Service</param>
        /// <param name="auctionService">Auction Service</param>
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer, bool notifyStoreOwner)
        {
            ...
            {
                //notification
                int orderCancelledCustomerNotificationQueuedEmailId = _workflowMessageService.SendOrderCancelledStoreOwnerNotification(order, order.CustomerLanguageId);
                if (orderCancelledCustomerNotificationQueuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
                        Note = string.Format("\"Order cancelled\" by customer has been queued. Queued email identifier: {0}.", orderCancelledCustomerNotificationQueuedEmailId),
                        DisplayToCustomer = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        private bool VendorNotification(Order order)
        {
            //notification for vendor
            foreach (var orderItem in order.OrderItems)
            {
                if (orderItem.Product.VendorId > 0)
                {
                    ...
                    {
                        _workflowMessageService.SendOrderCancelledVendorNotification(order, vendor, order.CustomerLanguageId);
                    }
                }
            }
        ...
        protected virtual void ProcessOrderPaid(Order order)
        {
            ...
            if (order.OrderTotal != decimal.Zero)
            {
                ...
                EngineContext.Current.Resolve<ICustomerActionEventService>().AddOrder(order, CustomerActionTypeEnum.PaidOrder);
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    foreach (var sc in details.Cart)
                    {
                        ...
                        if (sc.Product.ProductType == ProductType.ReservationProduct)
                        {
                        ...
                        if (sc.Product.ProductType == ProductType.ReservationProduct)
                        {
                            ...
                            {
                                var reservations = _productReservationService.GetProductReservationsByProductId(sc.Product.Id, true, null);
                                ...
                                foreach (var group in grouped)
                                {
                                    ...
                                    if (sc.Product.IncBothDate && sc.Product.IntervalUnitType == IntervalUnit.Day)
                                    {
                        ...
                        {
                            var bid = (_auctionService.GetBidsByProductId(sc.Product.Id)).Where(x => x.Amount == sc.Product.HighestBid).FirstOrDefault();
                            if (bid == null)
                        ...
                        if (sc.Product.ProductType == ProductType.AuctionProduct && sc.ShoppingCartType == ShoppingCartType.ShoppingCart)
                        {
                            _auctionService.UpdateAuctionEnded(sc.Product, true, true);
                            _auctionService.UpdateHighestBid(sc.Product, sc.Product.Price, order.CustomerId);
                            _workflowMessageService.SendAuctionEndedCustomerNotificationBin(sc.Product, order.CustomerId, order.CustomerLanguageId, order.StoreId);
                            _auctionService.InsertBid(new Bid()
                            {
                                CustomerId = order.CustomerId,
                                OrderId = order.Id,
                                Amount = sc.Product.Price,
                        ...
                        if (sc.Product.ProductType == ProductType.AuctionProduct && _orderSettings.UnpublishAuctionProduct)
                        {
                            sc.Product.Published = false;
                            _productService.UpdateProduct(sc.Product);
                        }
                    ...
                    EngineContext.Current.Resolve<ICustomerActionEventService>().AddOrder(order, CustomerActionTypeEnum.AddOrder);
                    //check order status
        ...
        /// <param name="order">Order</param>
        /// <param name="notifyCustomer">True to notify customer</param>
        /// <param name="notifyStoreOwner">True to notify storeOwner</param>
