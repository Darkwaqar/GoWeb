...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            int quantity)
        {
            ...
            if (product.ProductType != ProductType.SimpleProduct && product.ProductType != ProductType.AuctionProduct )
            {
        ...
        public IList<ShoppingCartItem> GetShoppingCart(int storeId = 0, params ShoppingCartType[] shoppingCartType)
        {
            IEnumerable<ShoppingCartItem> cart = _workContext.CurrentCustomer.ShoppingCartItems;

            if (storeId > 0)
                cart = cart.LimitPerStore(storeId);
            ...
            int quantity = 1, bool automaticallyAddRequiredProductsIfEnabled = true)
        {
            ...
            if (shoppingCartItem != null)
            {
                ...
                if (!warnings.Any())
                {
                    ...
                    EngineContext.Current.Resolve<ICustomerActionEventService>().AddToCart(shoppingCartItem, product, customer);
                }
            }
            else
            {
        ...
        /// <summary>
        /// Migrate Order shopping cart
        /// </summary>
        /// <param name="order">customer order</param>
        /// <param name="toCustomer">To customer</param>
        public virtual void MigrateOrderShoppingCart(Order order, Customer toCustomer)
        {
            if (order == null)
            ...
            //shopping cart items
            var fromCart = order.OrderItems.ToList();
            for (int i = 0; i < fromCart.Count; i++)
            {
                var sci = fromCart[i];
                AddToCart(toCustomer, sci.Product, ShoppingCartType.ShoppingCart, order.StoreId,
                    sci.AttributesXml, sci.PriceExclTax,
                    sci.RentalStartDateUtc, sci.RentalEndDateUtc, sci.Quantity, false);
            }
        }
