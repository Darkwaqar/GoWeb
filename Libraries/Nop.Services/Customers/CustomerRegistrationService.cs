...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerLoginResults ValidateCustomer(string usernameOrEmail, string password)
        {
            ...
            //2fa required
            if (customer.GetAttribute<bool>(SystemCustomerAttributeNames.TwoFactorEnabled) && _customerSettings.TwoFactorAuthenticationEnabled)
        ...
        /// <returns>Result</returns>
        public virtual CustomerLoginResults ValidateCustomer(string usernameOrEmail, string password, int storeId)
        {
            var customer = _customerSettings.UsernamesEnabled ?
                _customerService.GetCustomerByUsername(usernameOrEmail, storeId) :
                _customerService.GetCustomerByEmail(usernameOrEmail, storeId);
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            if (_customerService.GetCustomerByEmail(request.Email, request.StoreId) != null)
            {
            ...
            if (_customerSettings.UsernamesEnabled)
            {
                if (_customerService.GetCustomerByUsername(request.Username, request.StoreId) != null)
                {
        ...
        /// <returns>Result</returns>
        public virtual ChangePasswordResult ChangePassword(ChangePasswordRequest request, Customer customer)
        {
            if (request == null)
                throw new ArgumentNullException("request");
            ...
            if (customer == null)
            {
                customer = _customerService.GetCustomerByEmail(request.Email, request.StoreId);
                if (customer == null)
                {
                    result.AddError(_localizationService.GetResource("Account.ChangePassword.Errors.EmailNotFound"));
                    return result;
        ...
        /// <param name="requireValidation">Require validation of new email address</param>
        /// <param name="storeId">Store Id</param>
        public virtual void SetEmail(Customer customer, string newEmail, bool requireValidation, int storeId)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            var customer2 = _customerService.GetCustomerByEmail(newEmail, storeId);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new NopException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
        ...
        /// <param name="newUsername">New Username</param>
        /// <param name="storeId">Store Id</param>
        public virtual void SetUsername(Customer customer, string newUsername, int storeId)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            var user2 = _customerService.GetCustomerByUsername(newUsername, storeId);
            if (user2 != null && customer.Id != user2.Id)
                throw new NopException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
        ...
        #region ValidateCustomerByEmail

        public CustomerLoginResults ValidateCustomerByEmail(string usernameOrEmail, int storeId)
        {
            Customer customer;
            if (_customerSettings.UsernamesEnabled)
                customer = _customerService.GetCustomerByUsername(usernameOrEmail, storeId);
            else
                customer = _customerService.GetCustomerByEmail(usernameOrEmail, storeId);
