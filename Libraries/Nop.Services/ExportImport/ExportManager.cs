    ...
    public partial class ExportManager : IExportManager
    {
        ...
        public virtual string ExportNewsletterSubscribersToTxt(IList<NewsLetterSubscription> subscriptions)
        {
            ...
            foreach (var subscription in subscriptions)
            {
                ...
                sb.Append(subscription.VendorId);
        ...
        public virtual byte[] ExportCustomerGdprInfoToXlsx(Customer customer, int storeId)
        {
            ...
            var customerManager = new PropertyManager<Customer>(new[]
            {
                ...
                //attributes
                new PropertyByName<Customer>("First name", p => p.GetAttribute<string>(SystemCustomerAttributeNames.FirstName), !_customerSettings.FirstNameEnabled),
                new PropertyByName<Customer>("Last name", p => p.GetAttribute<string>( SystemCustomerAttributeNames.LastName), !_customerSettings.LastNameEnabled),
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                //customer addresses
                if (customer.Addresses is IList<Address> addresses && addresses.Any())
                {
                    customerInfoRow += 2;
                ...
                if (orders.Any())
                {
                    ...
                    foreach (var order in orders)
                    {
                        ...
                        //products
                        var orederItems = order.OrderItems;
        ...
        /// Export product list to xml
        /// </summary>
        /// <param name="coupons">Products</param>
        /// <returns>Result in XML format</returns>
        public virtual string ExportCouponsToXml(IList<Coupon> coupons)
        {
            var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            ...
            foreach (var coupon in coupons)
            {
                ...
                xmlWriter.WriteString("CouponId", coupon.Id);
                xmlWriter.WriteString("CouponType", coupon.CouponTypeId);
                xmlWriter.WriteString("Amount", coupon.Amount);
                xmlWriter.WriteString("IsCouponActivated", coupon.IsCouponActivated);
                xmlWriter.WriteString("CouponCouponCode", coupon.CouponCouponCode);
                xmlWriter.WriteString("RecipientName", coupon.RecipientName);
                xmlWriter.WriteString("RecipientEmail", coupon.RecipientEmail);
                xmlWriter.WriteString("SenderName", coupon.SenderName);
                xmlWriter.WriteString("SenderEmail", coupon.SenderEmail);
                xmlWriter.WriteString("Message", coupon.Message);
                xmlWriter.WriteString("IsRecipientNotified", coupon.IsRecipientNotified);
                xmlWriter.WriteString("CreatedOnUtc", coupon.CreatedOnUtc);
        ...
        public virtual byte[] ExportCouponsToXlsx(IEnumerable<Coupon> coupons)
        {
            var properties = new[]
            {
                new PropertyByName<Coupon>("CouponId", p => p.Id),
                new PropertyByName<Coupon>("CouponType", p => p.CouponTypeId)
                {
                    DropDownElements = CouponType.Virtual.ToSelectList(useLocalization: false)
                },
                new PropertyByName<Coupon>("Amount", p => p.Amount),
                new PropertyByName<Coupon>("IsCouponActivated", p => p.IsCouponActivated),
                new PropertyByName<Coupon>("CouponCouponCode", p => p.CouponCouponCode),
                new PropertyByName<Coupon>("RecipientName", p => p.RecipientName),
                new PropertyByName<Coupon>("RecipientEmail", p => p.RecipientEmail),
                new PropertyByName<Coupon>("SenderName", p => p.SenderName),
                new PropertyByName<Coupon>("SenderEmail", p => p.SenderEmail),
                new PropertyByName<Coupon>("Message", p => p.Message),
                new PropertyByName<Coupon>("IsRecipientNotified", p => p.IsRecipientNotified),
                new PropertyByName<Coupon>("CreatedOnUtc", p => p.CreatedOnUtc),
               
            };
            var couponList = coupons.ToList();

            return new PropertyManager<Coupon>(properties, _catalogSettings).ExportToXlsx(couponList);
        }
        ...
        /// </summary>
        /// <returns>Result in XML format</returns>
        public virtual string ExportPollCategoriesToXml()
        {
            var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            xmlWriter.WriteStartDocument();
            xmlWriter.WriteStartElement("PollCategories");
            xmlWriter.WriteAttributeString("Version", NopVersion.CurrentVersion);
            WriteCategories(xmlWriter, 0);
            xmlWriter.WriteEndElement();
            xmlWriter.WriteEndDocument();
        ...
        /// </summary>
        /// <param name="categories">Categories</param>
        public virtual byte[] ExportPollCategoriesToXlsx(IEnumerable<PollCategory> categories)
        {

            //var parentCatagories = new List<PollCategory>();
            //if (_catalogSettings.ExportImportCategoriesUsingPollCategoryName)
            //{
            //    //performance optimization, load all parent categories in one SQL request
            //    parentCatagories = _categoryService.GetCategoriesByIds(categories.Select(c => c.ParentCategoryId).Where(id => id != 0).ToArray());
            //}
            ...
            //property manager 
            var manager = new PropertyManager<PollCategory>(new[]
            {
                new PropertyByName<PollCategory>("Id", p => p.Id),
                new PropertyByName<PollCategory>("Name", p => p.Name),
                new PropertyByName<PollCategory>("Description", p => p.Description),
                new PropertyByName<PollCategory>("MetaKeywords", p => p.MetaKeywords, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("MetaDescription", p => p.MetaDescription, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("MetaTitle", p => p.MetaTitle, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("SeName", p => p.GetSeName(0), IgnoreExportCategoryProperty()),
                //new PropertyByName<PollCategory>("ParentCategoryName", p =>
                //{
                //    var category = parentCatagories.FirstOrDefault(c => c.Id == p.ParentCategoryId);
                //    return category != null ? _categoryService.GetFormattedBreadCrumb(category) : null;
                //}, !_catalogSettings.ExportImportCategoriesUsingCategoryName),
                new PropertyByName<PollCategory>("Picture", p => GetPictures(p.PictureId)),
                new PropertyByName<PollCategory>("PageSize", p => p.PageSize, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("AllowCustomersToSelectPageSize", p => p.AllowCustomersToSelectPageSize, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("PageSizeOptions", p => p.PageSizeOptions, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("ShowOnHomepage", p => p.ShowOnHomePage, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("IncludeInTopMenu", p => p.IncludeInTopMenu, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("Published", p => p.Published, IgnoreExportCategoryProperty()),
                new PropertyByName<PollCategory>("DisplayOrder", p => p.DisplayOrder)
            }, _catalogSettings);
