...
namespace Nop.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        public static bool IsValidNumber(string sendTestNumberTo)
        {
            if (String.IsNullOrEmpty(sendTestNumberTo))
                return false;

            sendTestNumberTo = sendTestNumberTo.Trim();
            var result = Regex.IsMatch(sendTestNumberTo, @"\s*(?:\+?(\d{1,3}))?([-. (]*(\d{3})[-. )]*)?((\d{3})[-. ]*(\d{2,4})(?:[-.x ]*(\d+))?)\s*", RegexOptions.IgnoreCase);
            return result;
        }
