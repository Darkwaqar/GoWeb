...
namespace Nop.Services.Tests.Discounts
{
    ...
    public class DiscountServiceTests : ServiceTest
    {
        ...
        public void Can_validate_discount_dateRange()
        {
            ...
            _discountService.ValidateDiscount(discount:discount, customer:customer, couponCodesToValidate: null).IsValid.ShouldEqual(true);

            discount.StartDateUtc = DateTime.UtcNow.AddDays(1);
            _discountService.ValidateDiscount(discount: discount, customer: customer, couponCodeToValidate: null).IsValid.ShouldEqual(false);
        }
    }
}
